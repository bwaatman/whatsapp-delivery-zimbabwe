...
import { OrderEconomicsService } from './OrderEconomicsService';
import { PaymentService } from './PaymentService';
import { CashReconciliationService } from './CashReconciliationService';
...
const logAuditEvent = async (action: string, userId: string, userType: string, details: any) => {
...
// Get wallet balance
router.get('/wallet/:userId/:userType', async (req: Request, res: Response) => {
  try {
    ...
    const wallet = await walletService.getOrCreateWallet(userId, userType as 'driver' | 'vendor' | 'platform');
...
router.get('/wallet/:walletId/transactions', async (req: Request, res: Response) => {
  try {
...
router.put('/config/platform/:key', rateLimit(10, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    // Log audit event
    await logAuditEvent('CONFIG_UPDATE', (req as any).userId || 'unknown', (req as any).userType || 'unknown', { key, value });
...
router.post('/economics/calculate', async (req: Request, res: Response) => {
  try {
...
router.post('/payment/online', async (req: Request, res: Response) => {
  try {
...
router.post('/payment/cash', async (req: Request, res: Response) => {
  try {
...
// Process refund
router.post('/payment/refund', async (req: Request, res: Response) => {
  try {
...
router.post('/payment/cancel', async (req: Request, res: Response) => {
  try {
...
router.get('/cash/debts/summary', async (req: Request, res: Response) => {
  try {
...
// Get driver debts
router.get('/cash/debts/:driverId', async (req: Request, res: Response) => {
  try {
...
router.post('/cash/debts/:debtId/writeoff', rateLimit(10, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    // Log audit event
    await logAuditEvent('DEBT_WRITEOFF', (req as any).userId || 'unknown', (req as any).userType || 'unknown', { debtId, reason });
...
router.post('/payouts/batch', async (req: Request, res: Response) => {
  try {
...
// Get payout batch
router.get('/payouts/batch/:batchId', async (req: Request, res: Response) => {
  try {
...
router.get('/payouts/batch/:batchId/payouts', async (req: Request, res: Response) => {
  try {
    const batchId = getParam(req.params.batchId);
...
router.put('/payouts/:payoutId/approve', rateLimit(20, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    ...
    // Log audit event
    await logAuditEvent('PAYOUT_APPROVE', (req as any).userId || 'unknown', (req as any).userType || 'unknown', { payoutId, approvedBy });
...
router.put('/payouts/:payoutId/reject', rateLimit(20, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    const payoutId = getParam(req.params.payoutId);
    const { reason } = req.body;
    ...
    // Log audit event
    await logAuditEvent('PAYOUT_REJECT', (req as any).userId || 'unknown', (req as any).userType || 'unknown', { payoutId, reason });
...
router.post('/payouts/request', rateLimit(5, 60000), async (req: Request, res: Response) => {
  try {
    ...
    const wallet = await walletService.getOrCreateWallet(userId, userType as 'vendor' | 'driver');
...
router.post('/payouts/batch/:batchId/process', rateLimit(5, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    const batchId = getParam(req.params.batchId);
...
router.get('/payouts/user/:userId/:userType', async (req: Request, res: Response) => {
  try {
    const userId = getParam(req.params.userId);
    const userType = getParam(req.params.userType);
    const limit = parseInt(req.query.limit as string) || 20;
    const payouts = await payoutService.getUserPayoutHistory(userId, userType as 'driver' | 'vendor', limit);
    res.json(payouts);
  } catch (error) {
...
// Get pending batches
router.get('/payouts/batches/pending', async (req: Request, res: Response) => {
  try {
...
router.post('/payouts/batch/:batchId/cancel', rateLimit(5, 60000), authenticateUser, async (req: Request, res: Response) => {
  try {
    const batchId = getParam(req.params.batchId);
