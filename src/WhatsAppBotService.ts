import { Client, LocalAuth, Message, Location } from 'whatsapp-web.js';
...
export interface CustomerSession {
  ...
  selectedProducts?: Array<{ id: string; name: string; quantity: number; variant?: { name: string; price_adjustment: number } }>;
...
export class WhatsAppBotService {
  ...
  private async createSession(phone: string): Promise<CustomerSession> {
...
Reply "menu" to see available categories.`;
  ...
  private async handleMenuSelectionState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
  ...
  private async handleVendorSelectionState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
    ...
    if (!isNaN(selection) && selection > 0 && selection <= vendors.length) {
      ...
      selectedVendor = vendors.find((vendor: any) => 
        vendor.name.toLowerCase().includes(messageBody)
      );
    }
  ...
  private async displayProducts(message: Message, vendorId: string) {
    try {
  ...
  private async handleProductSelectionState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
    ...
    const productsNeedingVariants = productsWithVariants?.filter(p => 
  ...
  private async promptForVariantSelection(message: Message, products: any[]) {
    let variantMenu = `🎨 *Select Variants*\n\n`;
    
    products.forEach((product, pIndex) => {
      variantMenu += `*${product.name}*\n`;
      if (product.product_variants && product.product_variants.length > 0) {
        product.product_variants.forEach((variant: any, vIndex: number) => {
  ...
  private async handleVariantSelectionState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
    ...
    session.productsNeedingVariants.forEach((product: any, index: number) => {
  ...
  private async handleNamePromptState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
  ...
  private async createOrder(message: Message, phone: string, session: CustomerSession) {
    try {
  ...
  private async promptForLocation(message: Message) {
  ...
  private async handleLocationMessage(message: Message, phone: string, location: any) {
    try {
      ...
      const deliveryFee = await orderEconomicsService.calculateDeliveryFee(
  ...
  private async handleLocationReceivedState(message: Message, phone: string, messageBody: string, session: CustomerSession) {
    ...
    if (messageBody === 'cash' || messageBody === 'card' || messageBody === 'ecocash') {
      session.paymentMethod = messageBody as 'cash' | 'card' | 'ecocash';
      this.sessions.set(phone, session);
  ...
  private async handleOrderTracking(message: Message, phone: string, messageBody: string) {
    try {
  ...
  async sendOrderStatusNotification(customerPhone: string, orderId: string, status: string): Promise<void> {
    try {
  ...
  async sendMessageToCustomer(phone: string, message: string): Promise<void> {
    try {
