...
export class PayoutService {
   ...
   */
  async generatePayoutBatch(startDate: Date, endDate: Date): Promise<PayoutBatch | null> {
    try {
      ...
      for (const wallet of wallets) {
        ...
        const { data: walletData, error: walletIdError } = await supabase
          .from('wallets')
          .select('id')
   ...
   */
  async getPayoutsForBatch(batchId: string): Promise<Payout[]> {
    try {
    ...
    try {
      ...
      for (const payout of payouts) {
        try {
          ...
          // Deduct from wallet
          await walletService.processPayout(
            payout.user_id,
            payout.user_type,
            payout.net_amount,
            payout.id!,
            `Payout batch ${batchId.substring(0, 8)}...`
          );
   ...
   */
  async getUserPayoutHistory(userId: string, userType: 'driver' | 'vendor', limit: number = 20): Promise<Payout[]> {
    try {
   ...
   */
  async getPendingBatches(): Promise<PayoutBatch[]> {
    try {
   ...
   */
  async cancelPayoutBatch(batchId: string): Promise<boolean> {
    try {
