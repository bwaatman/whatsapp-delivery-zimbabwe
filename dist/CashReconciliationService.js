...
class CashReconciliationService {
     ...
     */
    async getDriverTotalDebt(driverId) {
        try {
            console.log(`💰 Getting total debt for driver ${driverId}`);
            const { data: debts, error } = await database_1.supabase
                .from('driver_debts')
                .select('amount_owed')
     ...
     */
    async settleDebt(debtId, payoutId, amount) {
        try {
            console.log(`💰 Settling debt ${debtId} with payout ${payoutId}: ${amount}`);
            // Get debt details
            const { data: debt, error: debtError } = await database_1.supabase
                .from('driver_debts')
                .select('*')
                .eq('id', debtId)
                .single();
     ...
     * Write off debt
     */
    async writeOffDebt(debtId, reason) {
        try {
     ...
     */
    async reconcileCashForPeriod(startDate, endDate) {
        try {
            console.log(`💰 Reconciling cash for period ${startDate} to ${endDate}`);
            // Get all COD orders in the period
            const { data: orders, error: orderError } = await database_1.supabase
                .from('orders')
                .select('*')
            ...
            const { data: debts, error: debtError } = await database_1.supabase
                .from('driver_debts')
                .select('amount_owed')
                .gte('created_at', startDate.toISOString())
                .lte('created_at', endDate.toISOString())
    ...
    async getDebtSummary() {
        try {
            ...
            const { data: pendingDebts, error: pendingError } = await database_1.supabase
                .from('driver_debts')
                .select('amount_owed')
                .in('status', ['pending', 'partially_paid']);
            // Get total written off debt
            const { data: writtenOffDebts, error: writtenOffError } = await database_1.supabase
                .from('driver_debts')
                .select('amount_owed')
                .eq('status', 'written_off');
            // Get unique drivers with debt
            const { data: driversWithDebt, error: driverError } = await database_1.supabase
                .from('driver_debts')
                .select('driver_id')
                .in('status', ['pending', 'partially_paid']);
...
//# sourceMappingURL=CashReconciliationService.js.map