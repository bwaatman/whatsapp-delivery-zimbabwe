...
async function verifyDatabaseSchema() {
    ...
    try {
        ...
        const { data: postgisCheck, error: postgisError } = await exports.supabase
        ...
        const { data: merchantsCheck, error: merchantsError } = await exports.supabase
            .from('merchants')
            .select('id')
            .limit(1);
        if (!merchantsError) {
        ...
        // Check drivers table
        const { data: driversCheck, error: driversError } = await exports.supabase
            .from('drivers')
            .select('id')
            .limit(1);
        if (!driversError) {
        ...
        // Check orders table
        const { data: ordersCheck, error: ordersError } = await exports.supabase
            .from('orders')
            .select('id')
            .limit(1);
        if (!ordersError) {
...
async function testDatabaseOperations() {
    ...
    try {
        ...
        const { data: newMerchant, error: insertError } = await exports.supabase
            .from('merchants')
            .insert({
            name: 'Test Merchant',
        ...
        if (!insertError && newMerchant) {
            ...
            const { data: merchant, error: selectError } = await exports.supabase
                .from('merchants')
                .select('*')
            ...
            if (!selectError && merchant) {
                ...
                const { data: updatedMerchant, error: updateError } = await exports.supabase
                    .from('merchants')
                ...
                if (!updateError && updatedMerchant) {
                    ...
                    const { data: newDriver, error: driverError } = await exports.supabase
                        .from('drivers')
                        .insert({
                        name: 'Test Driver',
                        phone: '+263987654321',
                    ...
                    if (!driverError && newDriver) {
                        ...
                        // Clean up test data
                        await exports.supabase.from('drivers').delete().eq('id', newDriver.id);
                    }
                    else {
            ...
            // Clean up test data
            await exports.supabase.from('merchants').delete().eq('id', newMerchant.id);
        }
        else {
