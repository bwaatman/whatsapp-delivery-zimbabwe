...
exports.OrderEconomicsService = void 0;
...
class OrderEconomicsService {
    ...
    async getPlatformConfig() {
        try {
            ...
            const { data: configs, error } = await database_1.supabase
     ...
     */
    async calculateDeliveryFee(vendorLocation, customerLocation) {
        try {
     ...
     */
    async calculateOrderEconomics(foodAmount, deliveryFee) {
        try {
     ...
     */
    async updateOrderWithEconomics(orderId, economics) {
        try {
     ...
     */
    async processOrderCompletion(orderId, vendorId, driverId) {
        try {
            ...
            if (order.food_amount && order.delivery_fee) {
                ...
                const foodAmount = orderDetails.reduce((sum, item) => sum + (item.price * item.quantity), 0);
            ...
            if (driverId) {
                ...
                if (order.payment_method === 'cash' || order.payment_method === 'ecocash') {
                    ...
                    // Record driver debt
                    const { error: debtError } = await database_1.supabase
                        .from('driver_debts')
                        .insert({
                        driver_id: driverId,
                        order_id: orderId,
...
exports.OrderEconomicsService = OrderEconomicsService;
//# sourceMappingURL=OrderEconomicsService.js.map